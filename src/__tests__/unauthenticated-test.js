...
describe('unauthenticated', () => {
  ...
  it('can ping server', async () =>
    expect((await liveApi.ping()).ping).toBeTruthy());
  ...
  it('can get asset index', async () =>
    expect((await liveApi.getAssetIndex()).asset_index).toBeTruthy());
  ...
  it('can get residences', async () =>
    expect((await liveApi.getResidences()).residence_list).toBeTruthy());
  ...
  it('can subscribe to tick updates', async () =>
    expect((await liveApi.subscribeToTick('R_100')).tick).toBeTruthy());

  it('can subscribe to multiple ticks updates', async () =>
    expect(
      async () => await liveApi.subscribeToTicks(['R_25', 'R_50', 'R_100']),
    ).not.toThrow());

  it('can unsubscribe from all tick updates', async () =>
    expect(async () => await liveApi.unsubscribeFromAllTicks()).not.toThrow());

  it('can get tick history with no parameters', async () =>
    expect(
      async () => (await liveApi.getTickHistory('R_100')).history,
    ).not.toThrow());
  ...
  it('can get details about a landing company', async () =>
    expect(
      (await liveApi.getLandingCompanyDetails('costarica'))
  ...
  it('can unsubscribe from all price proposal updates', async () =>
    expect(
      async () => await liveApi.unsubscribeFromAllProposals(),
    ).not.toThrow());

  it('can get candles for a symbol', async () =>
    expect((await liveApi.getCandles('R_50')).candles).toBeTruthy());
  ...
  it('can get server time', async () =>
    expect((await liveApi.getServerTime()).time).toBeTruthy());

  it('can verify email', async () =>
    expect(
      (await liveApi.verifyEmail('address@example.com', 'account_opening'))
        .verify_email,
    ).toBeTruthy());

  it('can get website status', async () =>
    expect((await liveApi.getWebsiteStatus()).website_status).toBeTruthy());
});
